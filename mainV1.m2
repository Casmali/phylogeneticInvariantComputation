...
G = digraph ( {{1,5},{5,1},{2,6},{6,2},{5,6},{6,5},{5,7},{6,7},{6,7},{7,8},{8,7},{8,9},{8,10},{10,8},{3,10},{10,3},{10,9},{9,4},{4,9}} , EntryMode => "edges")
--
...
for i in treeOptions do(
    ...
    edge = {}; 
    checks = {};
    for j from 0 to #edgeList -1 do (
        
        a = 2*j;
        f = 2*j +1;
        --print(a,f);
        checks = append(checks, mod(i#(a) +i#(f) ,2)); 
    );
    ...
    if tree then (
        ...
        for k from 0 to length(i)-1 do(
            if i#k == 0 then (
                --HERE IS THE PROBLEM< THIS CODE ONLY WORKS FOR UNICYCLIC NETWORKS, JUST NEED A LIL MORE FOR LEVEL 1
                firstInd = floor(k/2) ;
                --this should be good as we have 0,1 for everyone, the next one is the one that is fucked up 

                edg = lift(mod(k,2),ZZ);
                --print("indecies");
                print( firstInd, edg,k);
                edgetoDelete = edgeList#firstInd#edg;
...
as = []
for i from 1 to 2*(length leafList) + (#edgeList-1) do(
    ...
    for j in treeList do (
        ...
        for k from 0 to #edgeSubscripts-1 do(
            if k == 0 then (
                ...
                sym = concatenate("a",toString(k+1),"_",toString(edgeSubscripts#(k)));
                --this next line is where our error is!!!! sooo... What is going on? 
                --print(prod);
                print(S);
                print(sym);
                tem = prod * S_sym;
...
probabilisticEDDegree Ideal := ZZ => I -{{1,5},{5,1},{2,6},{6,2},{5,6},{6,5},{5,7},{6,7},{6,7},{7,8},{8,7},{8,9},{8,10},{10,8},{3,10},{10,3},{10,9},{9,4},{4,9}}> (
